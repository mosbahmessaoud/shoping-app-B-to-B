...
def register_client(client_data: ClientCreate, db: Session = Depends(get_db)):
    ...
    existing_username = db.query(Client).filter(Client.username == client_data.username).first()
...
def login_client(login_data: ClientLogin, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token(data={"sub": str(client.id), "type": "client"})
...
def update_client_profile(
    ...
    if client_data.username and client_data.username != current_client.username:
        existing = db.query(Client).filter(Client.username == client_data.username).first()
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Ce nom d'utilisateur est déjà utilisé"
